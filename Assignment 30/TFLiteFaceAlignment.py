...
class CoordinateAlignmentModel():
    ...
    def _preprocessing(self, img, bbox, factor=3.0):
        ...
        Keyword Arguments
        ----------
        factor : max edge 2factor for bounding box cropping.
...
# lip:52,55,59,53,56,58,69,68,67,71,63,64
def zoom_effect(img,landmarks):
    ...
    img_2x=img_2x/255
    mask_2x=mask_2x/255
    img_target = img[int(y - (h * 0.5)) : int(y + h + (h * 0.5)), int(x - (w * 0.5)) : int(x + w + (w *0.5))]
    img_target = img_target/255
    foreground = cv2.multiply(mask_2x, img_2x)
    background = cv2.multiply(img_target , 1 - mask_2x[y * 2 : (y + h) * 2, x * 2 : (x + w) * 2])
...
if __name__ == '__main__':
    ...
    for pred in fa.get_landmarks(img, boxes):
        ...
        #     cv2.putText(img,str(i),tuple(p),cv2.FONT_HERSHEY_SIMPLEX,0.25,(0,0,255))
    # mask =mask/255
    # result =img*mask
    # # print(time.perf_counter() - start_time)
    img =zoom_effect(img, landmarks_left_eye)
    img =zoom_effect(img, landmarks_right_eye) 
    img =zoom_effect(img, landmarks_lip) 

    cv2.imwrite("resilt.jpg",img)
